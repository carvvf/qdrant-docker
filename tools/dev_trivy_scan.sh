...
scan_target() {
  ...
  run_trivy "${mode}" --scanners vuln --severity "${REPORT_SEVERITIES}" "${target}" \
    | tee "${REPORTS_DIR}/${timestamp}.${name}.txt"
  run_trivy "${mode}" --scanners vuln --severity "${REPORT_SEVERITIES}" \
    --format json --output "${json_report}" "${target}"

  set +e
  run_trivy "${mode}" --scanners vuln --severity "${FAIL_SEVERITIES}" \
    --exit-code 1 --quiet "${target}" >/dev/null
  scan_status=$?
  set -e
...
if scan_target repository fs /workspace; then
  :
else
  merge_status "$?"
fi

if scan_target image image "${IMAGE_REF}"; then
  :
else
  merge_status "$?"
fi
